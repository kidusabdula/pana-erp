...
export default function NewPurchaseOrderContent() {
  const { push: toast } = useToast();
  const router = useRouter();
  const searchParams = useSearchParams();
  const [loading, setLoading] = useState(false);
  const [optionsLoading, setOptionsLoading] = useState(true);
  const [options, setOptions] = useState<PurchasingOptions | null>(null);
  const [itemSearchQuery, setItemSearchQuery] = useState("");
  const [filteredItems, setFilteredItems] = useState<any[]>([]);
  const [showItemDropdown, setShowItemDropdown] = useState(false);

  // Get supplier or req from URL params if available
  const supplierParam = searchParams.get("supplier") || "";

  const [formData, setFormData] = useState({
    supplier: supplierParam,
    transaction_date: new Date().toISOString().split("T")[0],
    schedule_date: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
      .toISOString()
  ...
  const fetchOptions = async () => {
    ...
    try {
      const response = await fetch("/api/purchasing/options");
      if (response.ok) {
  ...
  const handleItemSelect = (item: any) => {
    setNewItem({
      ...
      qty: 1,
      rate: item.standard_rate, // Use standard buying rate if available, defaulting to standard_rate
    });
    setItemSearchQuery("");
    setShowItemDropdown(false);
  };
  ...
  const calculateTotals = () => {
    ...
    const tax = subtotal * 0.15; // 15% VAT, customizable
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.details || "Failed to create purchase order");
      }

      const data = await response.json();
      toast({
        title: "Success",
        description: `Purchase Order ${data.data.purchaseOrder.name} created successfully`,
      });

      router.push("/crm/purchase-orders"); // Redirect to CRM PO List
    } catch (error: any) {
      toast({
        variant: "error",
        title: "Error",
    ...
    <div className="min-h-screen bg-background p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center">
          <Button
            variant="ghost"
            onClick={() => router.push("/crm/purchase-orders")}
            className="mr-4"
          >
        ...
        <Card className="mb-6 bg-blue-50 border-blue-200">
          <CardContent className="p-4">
            <div className="flex items-center">
              ...
              <div>
                <p className="font-medium">Creating purchase order for:</p>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="schedule_date">Req. By Date *</Label>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="pt-4 border-t space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span>Req. By Date:</span>
                  <span className="font-medium">
                    {formData.schedule_date
                      ? new Date(formData.schedule_date).toLocaleDateString()
                      : "Not set"}
                  </span>
                </div>
              </div>
