...
import { ItemGroup, ItemGroupCreateRequest, ItemGroupUpdateRequest } from '@/types/item-group';
...
export async function POST(request: NextRequest) {
  return handleApiRequest<{ item_group: ItemGroup }>(
    withEndpointLogging('/api/stock/settings/item-group - POST')(async () => {
      ...
      const doc = await frappeClient.db.getDoc<ItemGroup>(DOCTYPE, data.item_group_name);
  ...
  return handleApiRequest<{ message: string }>(
    withEndpointLogging('/api/stock/settings/item-group - DELETE')(async () => {
      ...
      if (children.length > 0) {
        throw new Error('Cannot delete item group with child groups. Please delete or move child groups first.');
      }
      ...
      if (items.length > 0) {
        throw new Error('Cannot delete item group that is used in items. Please update or delete those items first.');
      }
